...
Public Class InterfaceWindow
   ...
   Public Sub New()
      Try
         ...
         If GetCommandLineArgs().Count > 1 Then
            ShapeView.Shape = LoadShape(GetCommandLineArgs().Last())
         End If
      ...
      Try
         With FileDialog
            If .ShowDialog() = DialogResult.OK Then
               ShapeView.Shape = LoadShape(.FileName)
               Me.Invalidate()
            End If
         End With
      Catch ExceptionO As Exception
   ...
   Private Sub InterfaceWindow_DragDrop(sender As Object, e As DragEventArgs) Handles MyBase.DragDrop
      Try
         With CType(e.Data.GetData(DataFormats.FileDrop), String())
            If .Count > 0 Then
               ShapeView.Shape = LoadShape(.Last())
               Me.Invalidate()
            End If
         End With
      Catch ExceptionO As Exception
   ...
   Protected Overrides Sub OnPaint(e As PaintEventArgs)
      Try
         ...
         For Each Vertex As Vertex3DStr In ShapeView.Shape.Vertices
            RotatedVertex = Rotate(Vertex, ShapeView.AngleX, AxesE.x)
            RotatedVertex = Rotate(RotatedVertex, ShapeView.AngleY, AxesE.y)
            RotatedVertex = Rotate(RotatedVertex, ShapeView.AngleZ, AxesE.z)
            Factor = ShapeView.Zoom / (RotatedVertex.Z + ShapeView.CameraZ)
         ...
         ShapeView.Shape.Lines.ForEach(Sub(Line) e.Graphics.DrawLine(Pens.White, DisplayedShape(Line.Start), DisplayedShape(Line.End)))
